  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 container mx-auto px-6">
        {/* Header */}
        <div
          className={`text-center mb-20 transition-[opacity,transform,filter] duration-700 ease-out
            ${isVisible ? "opacity-100 translate-y-0 blur-0" : "opacity-0 translate-y-4 blur-[1px]"}
          `}
        >
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          {services.map((service, i) => {
            ...
            const baseDelay = 0.12 + i * 0.08; // card stagger
            return (
              <article
                key={service.title}
                className={`relative group rounded-2xl border border-dashed border-construction-dark/15 bg-white/70 backdrop-blur-sm shadow-[var(--shadow-card)]
                  transition-[opacity,transform,box-shadow,filter] duration-700 ease-out
                  ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}
                  hover:shadow-[var(--shadow-premium)] hover:-translate-y-1
                `}
                style={{ transitionDelay: isVisible ? `${baseDelay}s` : "0s" }}
              ...
              >
                ...
                <div className="flex items-center justify-between px-8 pt-8">
                  <div className="flex items-center gap-4">
                    {/* Icon pop-in */}
                    <div
                      className={`bg-construction-green/10 w-14 h-14 rounded-xl grid place-items-center
                        transition-[background-color,transform,opacity,filter] duration-500 ease-out
                        ${isVisible ? "opacity-100 scale-100 blur-0" : "opacity-0 scale-90 blur-[0.5px]"}
                        group-hover:bg-construction-green
                      `}
                      style={{ transitionDelay: isVisible ? `${baseDelay + 0.05}s` : "0s" }}
                    >
                      <Icon
                        className={`w-8 h-8 text-construction-green transition-colors duration-300 group-hover:text-white`}
                      />
                    </div>

                    {/* Title slide-in */}
                    <h3
                      className={`text-2xl font-bold text-construction-dark transition-[opacity,transform,filter] duration-600 ease-out
                        ${isVisible ? "opacity-100 translate-y-0 blur-0" : "opacity-0 translate-y-1 blur-[0.5px]"}
                      `}
                      style={{ transitionDelay: isVisible ? `${baseDelay + 0.1}s` : "0s" }}
                    >
                      {service.title}
                    </h3>
                  </div>
                  <span
                    className={`text-[11px] tracking-widest uppercase text-construction-gray/80 transition-opacity duration-500
                      ${isVisible ? "opacity-100" : "opacity-0"}
                    `}
                    style={{ transitionDelay: isVisible ? `${baseDelay + 0.12}s` : "0s" }}
                  >
                ...
                {/* Divider (wipe) */}
                <div className="mx-8 my-6 h-px overflow-hidden">
                  <div
                    className={`h-px bg-gradient-to-r from-transparent via-construction-dark/10 to-transparent origin-left
                      transition-transform duration-700 ease-out
                      ${isVisible ? "scale-x-100" : "scale-x-0"}
                    `}
                    style={{ transitionDelay: isVisible ? `${baseDelay + 0.16}s` : "0s" }}
                  />
                </div>
                ...
                <div className="px-8 pb-8">
                  <p
                    className={`text-construction-gray leading-relaxed mb-6 transition-[opacity,transform,filter] duration-600 ease-out
                      ${isVisible ? "opacity-100 translate-y-0 blur-0" : "opacity-0 translate-y-1 blur-[0.5px]"}
                    `}
                    style={{ transitionDelay: isVisible ? `${baseDelay + 0.2}s` : "0s" }}
                  >
                    {service.description}
                  </p>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {service.features.map((feature, idx) => (
                      <div
                        key={idx}
                        className={`flex items-center text-sm text-construction-gray transition-[opacity,transform,filter] duration-500 ease-out
                          ${isVisible ? "opacity-100 translate-y-0 blur-0" : "opacity-0 translate-y-1 blur-[0.5px]"}
                        `}
                        style={{ transitionDelay: isVisible ? `${baseDelay + 0.26 + idx * 0.04}s` : "0s" }}
                      >
                        {/* blueprint bullet */}
                  ...
                  {/* Spec footer (fade up) */}
                  <div
                    className={`mt-6 grid grid-cols-3 divide-x divide-construction-dark/10 rounded-lg border border-dashed border-construction-dark/15 bg-white/60
                      transition-[opacity,transform,filter] duration-600 ease-out
                      ${isVisible ? "opacity-100 translate-y-0 blur-0" : "opacity-0 translate-y-1 blur-[0.5px]"}
                    `}
                    style={{ transitionDelay: isVisible ? `${baseDelay + 0.26 + service.features.length * 0.04 + 0.1}s` : "0s" }}
                  >
